...
// Implementations of constructors and functions for the SymType class
SymType::SymType(string type_, SymType *arrType_, int width_) : type(type_), width(width_), arrType(arrType_) {}

// Implementations of constructors and functions for the symbol class
symbol::symbol(string name_, string t, SymType *arrType, int width) : name(name_), value("-"), offset(0), nestedTable(NULL)
{
    type = new SymType(t, arrType, width);
    size = sizeOfType(type);
}

symbol *symbol::update(SymType *t)
{
...
// Implementations of constructors and functions for the SymTable class
SymTable::SymTable(string name_) : name(name_), tempCount(0) {}
...
symbol *SymTable::gentemp(SymType *t, string initValue)
{
    // Create the name for the temporary
    string name = "t" + convertIntToString(currSt->tempCount++);
    symbol *sym = new symbol(name);
    sym->type = t;
    ...
    currSt->table.push_back(*sym);
    return &(currSt->table.back());
}
...
void SymTable::update()
{
    ...
    for (list<SymTable *>::iterator iter = tableList.begin(); iter != tableList.end(); iter++)
    {
        (*iter)->update();
    }
}
...
{
    symbol *temp = SymTable::gentemp(new SymType(t));
...
expression *convertBoolToInt(expression *expr)
{
    if (expr->type == "bool")
    {
        expr->loc = SymTable::gentemp(new SymType("int"));
...
int main()
{
    ...
    globalST = new SymTable("Global"); // Create global symbol table
    currSt = globalST;                 // Make global symbol table the currently active symbol table
    blockName = "";
